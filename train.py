...
Trains BloodCellCNN on the BloodMNIST dataset. 
Uses inverse-frequency clqss weighting to correct for the 2.75 imabalance 
between the largest class (neutrophil) and smallest (lymphocyte) documented 
in assets/class_distribution.png.

Device handling automatically uses a GPU if available (torch.cuda.is_available()), 
...
class BloodMNISTDataset(Dataset):
    """
    Wraps cached BloodMNIST .npy tensors for use with 
    PyTorch's DataLoader.

    Augmentation is applied only when train=True.
    Reasoning: blood cells have no canonical orientation, 
    so flipping/rotating a cell image doesn't change its true label.
    """
...
def compute_class_weights(labels: np.ndarray, num_classes: int = 8) -> torch.Tensor:
    """
    Compute inverse-frequency clqss weights for CrossEntropyLoss, correcting 
    for BloodMNIST's 2.74x imbalance between the largest class (neutrophil)
    and smallest (lymphocyte) - see assets/class_distribution.png.

    weight[c] = total_samples / (num_classes * count[c])

    This will keeo the average weight near 1.0, so it rescales the relative 
